...
#' @param par_plot some graphical parameters which can be passed to the plot.
#'        See \strong{Details} section for further information.
#' @param legend_options a list with fur further arguments useful for
...
#' an illustration.
#'
#' \code{par_plot} admits some parameters of \code{\link[graphics]{par}}
#' function. The following \emph{has preestablished values}:
#' \itemize{
#' \item{ \code{mai}:}{the margins can be manipulated with \code{mar}. The right
#' margin has a value equals to 7.2 using \code{mar} and all new values take it
#' as reference value.}
...
#' \code{\link[graphics]{legend}} function. The following
#' \emph{has preestablished values}:
#' \itemize{
#' \item{\code{x, y}:}{legend is always located on the right side, outside the
#' plot.
#' \item{\code{x} and \code{y}} coordinates cannot be manipulated, instead of
#' this, it exists
#' the argument \code{pos}, which can take character or numeric values. In the
#' first case,
#' it can be \code{"top", "center"} and \code{"bottom"}, in the later, it can be
#' any value of the y-coordinate, between 0 and 1.}
#' \item{\code{legend}:}{text of the legend cannot be edited.}
#' \item{\code{pch}:}{cannot be manipulated, it depends on \code{pch} parameter
#' of the plot.}
#' \item{\code{col}:}{cannot be manipulated, it depends on \code{col} parameters
#' of the plot
#' and the curve_options.}
#' #' \item{\code{lty}:}{cannot be manipulated, it depends on \code{lty}
#' parameters of the plot
#' and the curve_options.}
#' #' \item{\code{lwd}:}{cannot be manipulated, it depends on \code{lwd}
#' parameters of the plot
#' and the curve_options.}
#' #' \item{\code{pt.cex}:}{cannot be manipulated, it depends on \code{cex}
#' parameter of
#' the plot.}
